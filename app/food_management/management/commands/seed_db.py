...
from food_management.models import Ingredient, Recipe, RecipeIngredient, IngredientInventory, PreparedFood, MealLog
...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    ...
    # Seed MealLog items
    if not MealLog.objects.exists():
      meal_log_items = []
      recipes = list(Recipe.objects.all())
      prepared_foods = list(PreparedFood.objects.all())

      for i in range(random.randint(10, 20)):
        is_recipe = random.random() < 0.5
        choice_list = recipes if is_recipe else prepared_foods
        choice = random.choice((choice_list))
        meal_log_items.append(
            MealLog(
                recipe=choice if is_recipe else None,
                prepared_food=choice if not is_recipe else None,
                meal_date=timezone.now().date() - timedelta(days=random.randint(1, 10)),
                meal_type=random.choice(self.MEAL_TYPE_CHOICES),
                notes=random.choice(self.DESCRIPTIONS)
            )
        )

      MealLog.objects.bulk_create(meal_log_items)
      self.stdout.write(self.style.SUCCESS(f"✓ Created {len(meal_log_items)} meal_log items"))
    else:
      self.stdout.write("✓ MealLog entries already exist")
